...
def students():
    ...
    cursor.execute("SELECT batch_id, batch_name FROM batches WHERE status != 'completed' ORDER BY batch_name ASC")
    active_batches = cursor.fetchall()

    cursor.close()
    conn.close()
    return render_template("admin/students.html", students=students_list, batches=active_batches, search_query=search_query)
